...
class IncomeHistoryList(BaseResource):
    def get(self):
        return {'income_history': self.db.cursor_to_list(self.db.root['income_history'].find())}
    ...
    def post(self):
        ...
        # check type_id
        try:
            if not self.db.root['types_of_income'].find_one({'_id': ObjectId(args['type_id'])}):
                raise Exception
            else:
                args['type_id'] = ObjectId(args['type_id'])
        except:
            abort(400, message='types_of_income_doesnt_exist')

        # add time
        args['time'] = datetime.now(tz=pytz.timezone('Europe/Moscow'))

        # insert
        self.db.root['income_history'].insert(args)
